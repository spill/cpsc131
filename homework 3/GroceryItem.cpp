...
///////////////////////// TO-DO (4) //////////////////////////////
    : _upcCode(std::move(other._upcCode)), _brandName(std::move(other._brandName)),
      _productName(std::move(other._productName)), _price(other._price) 
...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
    // Compare attributes in the correct order
    if (auto cmp = _upcCode <=> rhs._upcCode; cmp != 0)
        return cmp;
    if (auto cmp = _productName <=> rhs._productName; cmp != 0)
        return cmp;
    if (auto cmp = _brandName <=> rhs._brandName; cmp != 0)
        return cmp;
    if (auto cmp = floating_point_is_equal(_price, rhs._price) ? std::weak_ordering::equivalent : (_price < rhs._price ? std::weak_ordering::less : std::weak_ordering::greater); cmp != 0)
        return cmp;

    return std::weak_ordering::equivalent; // All attributes are equal
  /////////////////////// END-TO-DO (19) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    // Using std::quoted to read strings surrounded by quotes
    if ( stream >> std::quoted(productName) >> std::ws &&  // std::ws is used to consume white spaces
               ',' == stream.get() &&                  // Read and verify the comma separator
               stream >> std::ws >> std::quoted(brandName) >> std::ws &&
               ',' == stream.get() &&
               stream >> std::ws >> std::quoted(upcCode) >> std::ws &&
               ',' == stream.get() &&
               stream >> price )
    {
        // If reading was successful, move the read values into the groceryItem
        groceryItem = GroceryItem(std::move(upcCode), std::move(brandName), std::move(productName), price);
    }
    else
    {
        // If there was an error reading from the stream, set the stream's failbit
...
{
  ///////////////////////// TO-DO (22) //////////////////////////////
      stream << std::quoted(groceryItem.upcCode()) << ", " << std::quoted(groceryItem.brandName()) << ", "
           << std::quoted(groceryItem.productName()) << ", " << groceryItem.price();
    return stream;
