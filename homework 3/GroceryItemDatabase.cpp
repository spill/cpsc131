...
while (fin >> item)
{
    groceryItems.push_back(item);  // Add the item to the data store
}
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
///////////////////////// TO-DO (3) //////////////////////////////
  ...
  ///                    exercise, getting familiar with recursion is a goal.
      GroceryItem* GroceryItemDatabase::find(const std::string & upc) // find function 
{
    return findHelper(upc, 0); // cals private helper function 
}


GroceryItem* GroceryItemDatabase::findHelper(const std::string & upc, size_t index) //findHelper function (recursive)
{
    if (index >= groceryItems.size())
        return nullptr;  

    if (groceryItems[index].upcCode() == upc)
        return &groceryItems[index];   
    
    return findHelper(upc, index + 1);  
}
// checks if the index has gone beyond the size of the vector, returns nullptr if not found
// if upc matches desired upc at the index, returns pointer to that item
// if neither are met, recursively calls itself with incremented index 
...
// returns the total number of Grocery Item objects in the vector
/////////////////////// END-TO-DO (3) ////////////////////////////
