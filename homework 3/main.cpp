  ...
  *********************************************************************************************************************************/
  void carefully_move_grocery_items( std::size_t quantity, std::stack<GroceryItem> & broken_cart, std::stack<GroceryItem> & working_cart, std::stack<GroceryItem> & spare_cart )
  {
          ...
          if (quantity == 1)
    {
        ...
        broken_cart.pop(); // if quantity is 1, (only one item left to move), move the top item from broken_cart to working_cart
        trace(broken_cart, working_cart, spare_cart); // call the trace function with the current state
    }
    else
    {
        carefully_move_grocery_items(quantity-1, broken_cart, spare_cart, working_cart); // first recursively move quantity-1 items from broken cart to the spare cart, working cart as the intermediary
        working_cart.push(broken_cart.top()); // then top item from broken cart is moved to working cart, trace function recalled to output the current state 
        broken_cart.pop();
        trace(broken_cart, working_cart, spare_cart);
        carefully_move_grocery_items(quantity-1, spare_cart, working_cart, broken_cart); // this time spare cart to working cart using broken cart as intermediary
    }
    /////////////////////// END-TO-DO (1) ////////////////////////////
  }
...
if (!from.empty()) { 
    trace(from, to, std::stack<GroceryItem>()); // calls trace function to output the current state 
    std::stack<GroceryItem> spare_cart;
    carefully_move_grocery_items(from.size(), from, to, spare_cart);
}

      // starter function checks to see if there are any items in the from cart, 
      // records the state then initializes spare_cart and only after that begins the recursive process
    /////////////////////// END-TO-DO (2) ////////////////////////////
  }
}    // namespace
...
std::array<std::string, 6> upcCodes = {
    "00688267039317",   // eggs
    "00835841005255",   // bread
    "09073649000493",   // apple pie
    "00025317533003",   // hotdogs
    "00038000291210",   // rice krispies
    "00075457129000",   // milk
};  

std::array<std::string, 6> productNames = {
    "milk",
    "rice krispies",
    "hotdogs",
    "apple pie",
...
std::array<std::string, 6> brandNames = {
    "any",
    "any",
    "any",
    "Applegate Farms",
...
for (size_t i = 0; i < upcCodes.size(); ++i) // iterate over each of the arrays, increments until reaches 6 (size of the upcCodes array)
{
    myCart.push(GroceryItem(productNames[i], brandNames[i], upcCodes[i])); // pushes groceryItem object onto the myCart container
}
...
carefully_move_grocery_items(myCart, workingCart);
    ...
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      /// no charge.
for (const auto & upc : upcCodes) //starts a range based loop that iterates over each UPC code in the array 
{
    const GroceryItem* item = worldWideDatabase.find(upc); // searches for grocery item in worldWideDatabase, the find function returns a pointer to the Grocery Item, returns nullptr if nothing is found  

    
    std::cout << std::fixed << std::setprecision(2);

    if (item) {  //checks to see if item pointer is valid, (upc found in worldWideDatabase)
        std::cout << "\"" << upc << "\", "
                  << "\"" << item->brandName() << "\", "
                  << "\"" << item->productName() << "\", "
                  << item->price() << '\n';
              amountDue += item->price(); // adds price of the item to total
    } else {
       
        std::cout << "\"" << upc << "\" (apple pie) not found, so today is your lucky day - You get it free! Hooray!" << '\n';
    }
}
    /////////////////////// END-TO-DO (7) ////////////////////////////
