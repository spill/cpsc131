...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    ///        2) https://www.youtube.com/watch?v=Mu-GUZuU31A
            ...
            std::string upcCode; // string declaration for upcCode
            unsigned int quantity;// hold the quantity of the produt with unsigned integer
            while (fin >> std::quoted(upcCode) >> quantity) { // loop reading data from file stream, use quoted function to read upc code followed by a value 
           _inventoryDB[upcCode] += quantity; // associative container (std::map) mapping upc code 
    }
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
    ...
    for (const auto& [customerName, cart] : shoppingCarts) { // range based loop over shoppingCarts container that attaches customer names with their shopping carts.
        receipt << customerName << "'s shopping cart contains:\n"; // ostream object indicating the contents of each shopping cart.
        auto itemsSold = ringUpCustomer(cart, receipt); // calls function ringupCustomer with the customer cart and reciept stream,=
        todaysSales.merge(itemsSold); // merge into totdaysSales
        receipt << "-------------------------\n"; // Separator after each customer
    }
  /////////////////////// END-TO-DO (3) ////////////////////////////
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    std::map<std::string, unsigned int> itemCounts;

    // Count the occurrences of each item in the shopping cart
    for (const auto &[upc, groceryItem] : shoppingCart) {
        itemCounts[upc]++; // itterates over shopping Cart, increments count for each UPC in itemCounts
    }

    // Process each item in the shopping cart
    for (const auto &[upc, count] : itemCounts) { // iterate over ItemCounts to each unique item
        const auto *itemInDatabase = worldWideGroceryDatabase.find(upc);
        if (itemInDatabase == nullptr) {
            receipt << "\"" << upc << "\", apple pie not found, free on us!\n";
            continue;
        }

        // Formatting the output according to the required format: UPC, Brand Name, Product Name, Cost
    receipt << "\"" << upc << "\", " << "\"" << itemInDatabase->brandName() << "\", " 
            << itemInDatabase->productName() << ", " 
            << std::fixed << std::setprecision(2) << itemInDatabase->price() * count << '\n';

    amountDue += itemInDatabase->price() * count; // incremented by total cost of each item

    auto itInventory = _inventoryDB.find(upc);
    if (itInventory != _inventoryDB.end() && itInventory->second >= count) {
        itInventory->second -= count;
        purchasedGroceries.insert(upc);
    } else {
        receipt << "Not enough stock for Item UPC: " << upc << '\n';
    }
}

receipt << "-------------------------\n";
receipt << "Total  $" << std::fixed << std::setprecision(2) << amountDue << "\n\n";
  /////////////////////// END-TO-DO (4) ////////////////////////////
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
    ...
    for (const auto &upc : todaysSales) { // iterate through todaysSales holding the upc sold that day
        auto itInventory = _inventoryDB.find(upc);
        const auto *itemInDatabase = worldWideGroceryDatabase.find(upc);  // Use the variable here

        if (itInventory != _inventoryDB.end() && itInventory->second < REORDER_THRESHOLD) { //  if item is in the inventory and whether its stock amount is predefined
            int unitsBelowThreshold = REORDER_THRESHOLD - itInventory->second;
            reorderReport << "  " << "{" << "\"" << upc << "\",  \"" << (itemInDatabase ? itemInDatabase->brandName() : "Brand Unknown") 
                          << "\",  \"" << (itemInDatabase ? itemInDatabase->productName() : "Product Unknown") 
                          << "\",  " << (itemInDatabase ? std::to_string(itemInDatabase->price()) : "Price Unknown") << "}" << "\n";
            reorderReport << "        only " << itInventory->second << " remain in stock which is " << unitsBelowThreshold 
                          << " unit(s) below reorder threshold (" << REORDER_THRESHOLD << "), re-ordering " << LOT_COUNT << " more\n\n";
            itInventory->second += LOT_COUNT;
        } else if (itemInDatabase == nullptr) { // if item is below threshold, calculate unitsBelowThreshold. tells how many units in stock. 
            reorderReport << "  " << "{" << "\"" << upc << "\" *** no longer sold in this store and will not be re-ordered}\n\n";
        }
    }

    todaysSales.clear(); // empty container
  /////////////////////// END-TO-DO (5) ////////////////////////////
}
