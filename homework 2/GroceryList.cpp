...
std::size_t GroceryList::size() const
{
  ...
  ///////////////////////// TO-DO (1) //////////////////////////////
    ...
    return _gList_vector.size(); // Return size of the vector since all containers have the same size.
  /////////////////////// END-TO-DO (1) ////////////////////////////
}
...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  auto itemIterator = std::find(_gList_vector.begin(), _gList_vector.end(), groceryItem); // function finds a groceryitem in the grocerylist
 // locate the grocery item using std::find in gList_vector 
return std::distance(_gList_vector.begin(), itemIterator); // if the item iterator points to the correct item, provides a zero position, but if itemIterator points to the end aka (item isnt found)
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  // Prevent duplicate entries
  if(find(groceryItem) != size()) // return position if its found or the size of the list if not found 
    return; // ------------------^ if true return, thus preventing duplicates
  /////////////////////// END-TO-DO (3) ////////////////////////////
  ...
  { /**********  Part 1 - Insert into array  ***********************/
   ...
   if (_gList_array_size >= _gList_array.size())// first lets verify that the gList_array has a fixed capacity, the function checks to see if theres space remaining in the array
          throw CapacityExceeded_Ex("Array Capacity has Exceeded" exception_location); // I do this by comparing _gList_array_size which tracks the number of elements in the array against the _gList_array.size which is the array's capacity. -- if no space throw exception

      std::move_backward(_gList_array.begin() + offsetFromTop, _gList_array.begin() + _gList_array_size, _gList_array.begin() + _gList_array_size + 1); // check to see if there is space in the array
      _gList_array[offsetFromTop] = groceryItem; 
      ++_gList_array_size; // increment the array size
    /////////////////////// END-TO-DO (4) ////////////////////////////
  } // Part 1 - Insert into array
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      /// in your array, so don't forget to adjust _gList_array_size.
if(offsetFromTop < _gList_array_size) 
{
    // If removing from the top, shift everything left
    if (offsetFromTop == 0) 
    {
        std::move(_gList_array.begin() + 1, _gList_array.begin() + _gList_array_size, _gList_array.begin());
    } 
    // For all other positions, shift elements after the remove position to the left
    else 
    {
        std::move(_gList_array.begin() + offsetFromTop + 1, _gList_array.begin() + _gList_array_size, _gList_array.begin() + offsetFromTop);
    }
    _gList_array[_gList_array_size - 1] = GroceryItem();  // Clear the last element
    --_gList_array_size;
}
    /////////////////////// END-TO-DO (8) ////////////////////////////
  } // Part 1 - Remove from array
  ...
  if(offsetFromTop < _gList_dll.size()) 
  {
    auto dll_it = std::next(_gList_dll.begin(), offsetFromTop);
    _gList_dll.erase(dll_it);
  }
    /////////////////////// END-TO-DO (10) ////////////////////////////
  } // Part 3 - Remove from doubly linked list
  ...
  {/**********  Part 4 - Remove from singly linked list  **********/
    ///////////////////////// TO-DO (11) //////////////////////////////
      ...
      /// advancing _gList_sll.before_begin() offsetFromTop times.  The STL has a function called std::next() that does that, or you
      /// can write your own loop.
  if(offsetFromTop < gList_sll_size()) 
  {
    auto sll_prev_it = std::next(_gList_sll.before_begin(), offsetFromTop);

    _gList_sll.erase_after(sll_prev_it);
  }
    /////////////////////// END-TO-DO (11) ////////////////////////////
  } // Part 4 - Remove from singly linked list
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ...
  const std::size_t position = find(groceryItem);
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ...
  for(const auto& item : rhs._gList_vector) // range based loop which I used to iterate through each item in the container
  {

    insert(item, Position::BOTTOM); // for every item in the vector i call the insert function, i did it this way so i can insert into the bottom of the list (Position::BOTTOM) in the list. 
  }
  /////////////////////// END-TO-DO (14) ////////////////////////////
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    auto common_extent = std::min(size(), rhs.size()); // returns the size of the rhs list, I used the std::min function to return the 2 smaller values

    for(std::size_t i = 0; i < common_extent; ++i) // looped through items of each list, 3 way comparison (spaceship operator) 
    {
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    if(size() != rhs.size()) // checks for size difference
        return false;

    // O(n) operation: Check each item
    for(auto iterator1 = _gList_vector.begin(), iterator2 = rhs._gList_vector.begin();  // compares the individual grocery items 
        iterator1 != _gList_vector.end() && iterator2 != rhs._gList_vector.end(); 
        ++iterator1, ++iterator2)

    {

        if(!(*iterator1 == *iterator2)) // if the items are difference, return false
            return false;
    }
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  while(stream >> item) // will continue to read as-long as there are objects from GroceryItem to read from the input stream 
  {

    groceryList.insert(item, GroceryList::Position::BOTTOM); // appends to the bottom
  }
  /////////////////////// END-TO-DO (18) ////////////////////////////
